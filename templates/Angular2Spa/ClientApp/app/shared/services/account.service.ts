...
export class AccountService {
    constructor(private http: Http, @Inject(LocalStorage) private localStorage) {
        this.tokenCheck();
    }

    private registerURL = "api/Account/Register";
    private loginURL = "api/connect/token";
    ...
    tokenExpirationDate(): Date {
        return new Date(parseInt(this.localStorage.getItem(this.storageExpirationDate)));
        //LocalStorage.getItem(this.storageExpirationDate);
    }

    //get token from memory
    getToken(): string {
        if (this.tokenCheck()) {
            return this.localStorage.getItem(this.storageTokenKey);
        }
        else {
    ...
    tokenCheck(): boolean {
        if (this.localStorage.getItem(this.storageTokenKey) != undefined) {
            ...
            if (this.tokenExpirationDate().getTime() > new Date().getTime()) {
                ...
                //expired, delete it
                this.localStorage.removeItem(this.storageTokenKey);
                this.localStorage.removeItem(this.storageExpirationDate);
    ...
    getUserName(): string {
        return this.localStorage.getItem(this.storageUserName);
    }
